...
const ExchangeModal = ({ onClose, tokenData }: ExchangeModalProps) => {
  ...
  const handleRedeem = async () => {
    ...
    if (!tokensToExchange || isNaN(Number(tokensToExchange)) || Number(tokensToExchange) <= 0) {
      return;
    }
    try {
  ...
  // Calcula el monto de ETH que se recibirá
  const claimEth =
    tokensToExchange && !isNaN(Number(tokensToExchange))
      ? (Number(tokensToExchange) * Number(tokenData.exchangeRate)).toFixed(4)
      : "0";
  ...
  return (
    <Modal onClose={onClose}>
      <div className="flex flex-col items-center space-y-4 p-4">
        ...
        <p className="text-gray-500 text-lg">Your balance: {tokenData.balance}</p>
        <p className="text-gray-500 text-lg">
          Exchange Rate: 1 {tokenData.tokenSymbol} = {tokenData.exchangeRate} ETH
        </p>
        ...
        <button
          ...
          disabled={isSubmitting || !tokensToExchange || Number(tokensToExchange) <= 0}
        >
